...
class Lesson < ApplicationRecord
  ...
  def video_url
    format = CATEGORY_VIDEOS[self.category]
    format && format % video_id
  end

  def thumbnail_url
    format = CATEGORY_THUMBNAILS[self.category]
    format && format % video_id
  end
  ...
  def check_video_link
    ...
    errors[:video_id] << 'Must be a YouTube or Vimeo link e.g https://youtu.be/z1aIdcb43RE'
  end
end
